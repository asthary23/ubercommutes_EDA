...
```{r}
#Distribution for trips by base (apr.Base vector)
bases_group <- new_df %>%
...
location_group <- new_df %>%
  ...
  annotate("text", x = -73.6, y = 40.92, label = "Upby Brooklyn Area") + 
  theme_clean()
location_group
```
...
#Normalized stacked bar chart for proportions and association (vector(s): time_category, long_west (ordered and renamed levels))
prop_group <- new_df %>%
  ...
  labs(x = "Time of Day",
       y = "Number of Trips",
       title = "Trips by Time of Day on Easter (NYC)") +
  scale_fill_manual(name = "Trip from Upby Brooklyn",
                    labels = c("Yes", "No"),
